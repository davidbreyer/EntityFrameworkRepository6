...
namespace EntityFramework.Auditable.Repository6
{
            ...
            where TContext : AuditDbContext
    {
        ...
        protected AuditableBaseContext(string connectionString, IDatabaseInitializer<TContext> initializerStrategy) : base(connectionString)
        {
            Database.SetInitializer<TContext>(initializerStrategy);
        }

        protected AuditableBaseContext(DbConnection dbConnection, IDatabaseInitializer<TContext> initializerStrategy) : base(dbConnection, true)
        {
            Database.SetInitializer<TContext>(initializerStrategy);
        }

        protected AuditableBaseContext(DbConnection dbConnection, IDatabaseInitializer<TContext> initializerStrategy, bool contextOwnsConnection) : base(dbConnection, contextOwnsConnection)
        {
            Database.SetInitializer<TContext>(initializerStrategy);
        }
    }
